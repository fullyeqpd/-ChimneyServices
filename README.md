...
`npm run cards` draws the cards, writing SVG and PNG for five faces into `public/pro/cards/`: `art-kalina-front`, `art-kalina-back`, `art-kalina-simple`, `sample-front`, `sample-back`. Commit the output — the build does not regenerate it.
...
- `art-kalina` is a real record. `sample` is fictional on purpose — CS-P-00000, "Sample Technician", "Example Chimney Co.", a drawn silhouette rather than any person's face, and a SAMPLE watermark — so a layout demo can never be mistaken for a person. The sample faces are not shown on any page; they stay in the repo as the layout reference for the full card.
- `art-kalina-simple` is the one shown on `/professionals` and on `/pro/art-kalina`: name, two certification lines, and a QR code. No photograph, no date, no disclaimer strip — the record page the code opens carries all of that, and a card small enough to hand over should not repeat it.
